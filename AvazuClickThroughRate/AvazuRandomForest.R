...
rfmodel <- randomForest(trainA, y = ytrain, xtest = valA, ytest = yval, ntree = 100,replace=FALSE, nodesize = 10, maxnodes = 20,importance = TRUE, norm.votes = TRUE, do.trace = TRUE,keep.forest = TRUE,keep.inbag=FALSE)

# Investigate model performance
plot(rfmodel)

# Make predictions for validation set score
click <- predict(rfmodel, valA, type='prob')
click <- click[ ,2]
llscore <- llfun(as.numeric(yval), click)


# Make predictions for test set
click <- predict(rfmodel, testA, type='prob')
click <- click[ ,2]

# make into dataframe and write output file
id <- idtest
ytest <- data.frame(id,click)
write.csv(ytest, file = "randForR9preds100trees.csv", row.names=FALSE, quote = FALSE)
# Kaggle Score  5.2439805 absolutely horrible

# Build second model with train dataframe increase the number of brances for each tree
rfmodel2 <- randomForest(trainA, y = ytrain, xtest = valA, ytest = yval, ntree = 100,replace=FALSE, nodesize = 10, maxnodes = 100,importance = TRUE, norm.votes = TRUE, do.trace = TRUE,keep.forest = TRUE,keep.inbag=FALSE)

# Make predictions for test set
click <- predict(rfmodel2, testA, type='prob')
click <- click[ ,2]

# make into dataframe and write output file
id <- idtest
ytest <- data.frame(id,click)
write.csv(ytest, file = "randForR9preds100trees100nodes.csv", row.names=FALSE, quote = FALSE)
